      ...
      <box width={contentWidth()} flexGrow={1} flexDirection="column" paddingTop={1} paddingBottom={1} gap={1}>
        ...
        <box flexDirection="column" alignItems="center" flexShrink={0}>
          <text attributes={TextAttributes.BOLD} fg={colors.text}>
            DataForge stopped unexpectedly
          </text>
        ...
        <Show when={showFooter()}>
          <box flexDirection="column" alignItems="center" flexShrink={0}>
            <text fg={colors.muted}>
              {copied()
                ? "Report copied — paste it into your DataForge issue tracker."
                : "Copy the report and open your DataForge issue tracker to continue debugging."}
            </text>
            <text fg={colors.muted}>DataForge {InstallationVersion}</text>
          </box>
        </Show>
      </box>
    </box>
...
function buildIssueURL(message: string, stack: string) {
  ...
  const url = new URL(process.env.DATAFORGE_ISSUES_URL ?? "https://github.com/anomalyco/dataforge/issues/new")
  url.searchParams.set("title", `TUI crash: ${message}`)
  url.searchParams.set("dataforge-version", InstallationVersion)
  url.searchParams.set("os", describeOS())
  url.searchParams.set("terminal", describeTerminal())
  url.searchParams.set(
    "reproduce",
    "Reported automatically from the DataForge crash screen. If you can, describe what you were doing when it stopped.",
  )
  ...
  const marker = "\n... (truncated)"
  const head = `The DataForge terminal agent stopped with an unexpected error.\n\n**Error:** ${message}\n\n**Stack trace:**\n`
