    ...
    const value = await DialogPrompt.show(dialog, "Other", {
      ...
      description: () => (
        <text fg={theme.textMuted}>
          This stores the credential securely. DataForge uses it for Big Pickle and does not print it in logs.
        </text>
      ),
    })
    ...
    <DialogPrompt
      ...
      onConfirm={async (value) => {
        ...
        if (props.custom && !sync.data.provider_next.all.some((provider) => provider.id === props.providerID)) {
          toast.show({
            variant: "info",
            message: `Saved credential for ${props.providerID}. DataForge will use Big Pickle.`,
          })
          dialog.clear()
          return
        }
