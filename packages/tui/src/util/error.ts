...
export function cliErrorMessage(input: unknown): string | undefined {
  ...
  if (model) {
    ...
    return [
      ...
      "Try: `dataforge models` to list available models",
      "Or check your config (opencode.json) provider/model names; DataForge defaults to opencode/big-pickle",
    ].join("\n")
  }
  ...
  if (isRecord(input) && named(input, "MCPFailed")) {
    ...
    return `MCP server "${name}" failed. Note, DataForge requires MCP credentials to be configured in the project settings.`
  }
  return undefined
}
