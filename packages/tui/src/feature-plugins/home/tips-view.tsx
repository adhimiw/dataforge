...
const NO_MODELS_TIP =
  "Run {highlight}/connect{/highlight} to connect DataForge Zen, then use {highlight}/init{/highlight} to prepare this workspace"
...
const TIPS: Tip[] = [
  "Type {highlight}@{/highlight} followed by a filename to attach a dataset, notebook, or source file",
  "Start a message with {highlight}!{/highlight} to run a shell command (e.g., {highlight}!python -V{/highlight})",
  (shortcuts) => press(shortcuts.agentCycle(), "to switch between DataForge and Plan agents"),
  "Use {highlight}/undo{/highlight} to revert the last message and file changes",
  "Use {highlight}/redo{/highlight} to restore previously undone messages and file changes",
  "Run {highlight}/share{/highlight} only when you intentionally want to publish a session",
  "Drag and drop images or PDFs into the terminal as context",
  (shortcuts) => press(shortcuts.inputPaste(), "to paste images from your clipboard into the prompt"),
  (shortcuts) => `Use ${commandText("/editor", shortcuts.editorOpen())} to compose messages in your external editor`,
  "Run {highlight}/init{/highlight} to create workspace guidance and durable DataForge state",
  ...
  "Run {highlight}/connect{/highlight} to add your Zen credential; DataForge defaults to free Big Pickle",
  ...
  "Switch to {highlight}Plan{/highlight} for read-only suggestions without making changes",
  ...
  "Create {highlight}opencode.json{/highlight} for project settings, and {highlight}tui.json{/highlight} for the terminal UI",
  "Place terminal UI settings in {highlight}~/.config/opencode/tui.json{/highlight} for global config",
  ...
  "Create a plugin to prevent DataForge from reading sensitive files",
  "Use {highlight}dataforge run{/highlight} for non-interactive analysis or verification",
  "Use {highlight}dataforge --continue{/highlight} to resume the last session",
  "Use {highlight}dataforge run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}dataforge serve{/highlight} for headless API access to DataForge",
  "Use {highlight}dataforge run --attach{/highlight} to connect to a running server",
  "Run {highlight}dataforge upgrade{/highlight} to update to the latest version",
  "Run {highlight}dataforge auth list{/highlight} to inspect configured credentials without printing secrets",
  "Run {highlight}dataforge agent create{/highlight} for guided agent creation",
  "Use {highlight}/dataforge{/highlight} in GitHub issues/PRs only if your workflow enables the integration",
  "Run {highlight}dataforge github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/dataforge fix this{/highlight} only in repositories where automation is explicitly enabled",
  "Comment {highlight}/df{/highlight} on PR code lines for targeted code reviews",
  ...
  "Run {highlight}dataforge debug config{/highlight} to troubleshoot configuration",
  ...
  "Run the DataForge container image with your project mounted when you need an isolated runtime",
  "Use {highlight}/connect{/highlight} with DataForge Zen; Big Pickle is the default free model",
