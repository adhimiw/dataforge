  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    const options = [
      ...pipe(
        ...
        map((x) => ({
          ...
          value: x.id,
          hint: {
            opencode: "DataForge Zen · Big Pickle",
    ...
    if (provider === "other") {
      ...
      yield* Prompt.log.warn(
        `This stores a credential for ${provider}; DataForge will not print it in logs. Configure custom providers in opencode.json and check the docs for examples.`,
      )
    }
    ...
    if (provider === "opencode") {
      yield* Prompt.log.info(
        "Create a Zen API key at https://opencode.ai/auth. DataForge will use free Big Pickle by default.",
      )
    }
